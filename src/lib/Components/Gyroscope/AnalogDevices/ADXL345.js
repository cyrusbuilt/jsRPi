 ...
 */
class ADXL345 extends MultiAxisGyro {
    /**
     * Initializes a new instance of the jsrpi.Components.Gyroscope.AnalogDevices.ADXL345
     ...
     * @const
     */
    static get ADXL345_ADDRESS() { return ADXL345_ADDR; }
    ...
    enable() {
        if (this.isDisposed) {
            throw new ObjectDisposedException("jsrpi.Components.Gyroscope.AnalogDevices.ADXL345");
        }
        let packet = [ 0x31, 0x0B ];
    ...
    readGyro() {
        ...
        this._device.writeByte(this._address, 0x08);
        let self = this;
        let addr = this._address;
        let dev = this._device;
        CoreUtils.sleep(10).then(() => {
           let data = dev.readBytes(addr, 6);
           if (data.length !== 6) {
               throw new IOException("Couldn't read compass data; Return buffer size: " + data.length.toString());
           }

           self.aX.rawValue = ((data[0] & 0xff) << 8) + (data[1] & 0xff);
           self.aY.rawValue = ((data[2] & 0xff) << 8) + (data[3] & 0xff);
           self.aZ.rawValue = ((data[3] & 0xff) << 8) + (data[5] & 0xff);
        });
    }
