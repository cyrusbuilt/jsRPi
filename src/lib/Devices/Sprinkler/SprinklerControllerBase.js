...
function SprinklerControllerBase() {
  ...
  this.onSprinklerStateChange = function(sprinklerStateChangeEvent) {
    process.nextTick(function() {
      self.emit(SprinklerZone.EVENT_STATE_CHANGED, sprinklerStateChangeEvent);
    });
  };
  ...
  this.isOn = function() {
    var result = false;
    for (var i = 0; i < self.getZoneCount(); i++) {
      result = self.getZones()[i].isOn();
      if (result) {
        break;
      }
    }
  ...
  this.turnOffAllZones = function() {
    for (var i = 0; self.getZoneCount(); i++) {
      if (_zones[i].isOn()) {
        _zones[i].turnOff();
...
module.exports = extend(true, SprinklerControllerBase, DeviceBase, EventEmitter);
